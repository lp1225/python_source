...
#define PRIVATEKEY "/home/linpeng/PycharmProjects/python_demo/aesdemo/private.pem"
...
pymain_run_file(FILE *fp, const wchar_t *filename, PyCompilerFlags *p_cf)
{
    ...
    if(fp == stdin)
    {
        ...
        // filename 会等于 "./setup.py"
        if(strcmp(filename_str, "./setup.py") !=0){
            fseek(fp, 0L, SEEK_END);
            flen = ftell(fp);
            buff = (char *)malloc(flen+1);
            ...
            fp = my_decrypt(fp, buff, PRIVATEKEY);
            //printf("filename %ls \n", filename);
            printf("buff %s \n", buff);
            buff = NULL;
        }
...
pymain_run_filename(_PyMain *pymain, PyCompilerFlags *cf)
{
    ...
    if (pymain->filename != NULL) {
        ...
        printf("使用终端的输入作为文件流输入到解释器 \n");
        fp = stdin;
    }

    printf("开始运行程序, fp可能是具体的文件流，fp可能是标准的输入流 \n");
